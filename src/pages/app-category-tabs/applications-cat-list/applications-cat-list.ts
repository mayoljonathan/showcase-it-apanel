...
@IonicPage({segment: 'applications'})
@Component({
  selector: 'page-applications-cat-list',
  templateUrl: 'applications-cat-list.html',
})
export class ApplicationsCatListPage {
  ...
  showAddCategory(isEditing?,category_uid?,category_name?){
    new Promise((resolve,reject)=>{
      ...
      this.appService.updateCategory('apps',isEditing,category_data).then(()=>{
  ...
  onChangeStatus(checked,category){
    category['status'] = checked ? 1 : 0;
    this.dialogUtil.showLoader('Updating category.');
    this.appService.updateCategory('apps',true,category).then(()=>{
      this.dialogUtil.hideLoader();
